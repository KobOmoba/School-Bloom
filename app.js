...
async function doLogin(){
  ...
  try{
    ...
    // STEP 2: If not in schools collection, check admin_approved_schools
    // This handles schools approved before the direct-write fix
    if(!school){
      try{
        ...
        if(!snap.empty){
          ...
          if(rec.password===pwd){
            // Create the school document now so future logins work
            const newDoc={
              ...
              staff:[{name:'Principal',email:rec.principalEmail||(sid.toLowerCase()+'@bloom.edu.ng'),password:rec.password,role:'Principal',phone:rec.principalPhone||''}],
            ...
            try{ await db.collection('schools').doc(sid).set(newDoc,{merge:true}); }catch(e2){}
            school=newDoc;
          } else {
      ...
      }catch(e){ console.warn('admin_approved_schools check failed:', e); }
    }

    // STEP 3: Try localStorage cache (offline fallback)
    if(!school){
    ...
    if(!school){
      err.textContent='School ID not found. Check the ID sent by your agent.';
      err.style.display='block';btn.textContent='▶ Login';btn.disabled=false;return;
    }

    // Match user: try email+password, then password only
