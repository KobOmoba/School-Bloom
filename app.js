...
// null = sees everything. Explicit Set = whitelist of allowed tabs.
// Class Teacher and Subject Teacher: NO revenue, NO fees, NO finance, NO payroll.
// Only Bursar, Principal, and Proprietor can see money-related sections.
const ROLE_TABS = {
  'Principal':       null,
...
const ROLE_NORMALISE = {
  ...
  'proprietor': 'Proprietor', 'owner': 'Proprietor', 'director': 'Proprietor',
  'proprietress': 'Proprietor', 'chairman': 'Proprietor',
};
...
// Only Bursar, Principal, and Proprietor can see fee amounts, Revenue section, and payment receipts.
// Class Teachers and Subject Teachers are BLOCKED from all fee information.
function canSeeFees() { return ['Principal', 'Bursar', 'Proprietor'].includes(userRole); }
...
function showStaffLoginStep() {
  ...
  if (nameEl) nameEl.textContent = SD.config?.schoolName || 'Edu-BLOOM';
  slSetTab('principal');
}
...
function slForgotPassword() {
  ...
  const msg = 'Hello, I am the Principal of ' + school + '. I cannot log into Edu-BLOOM — please send my school password. School ID: ' + (schoolId || 'unknown');
...
function startApp() {
  ...
  const name = SD.config.schoolName || schoolId || 'Edu-BLOOM';
  ...
  // Start receipt listener for authorized roles
  if (canSeeFees()) _startReceiptListener();
  if (typeof renderBirthdays === 'function') renderBirthdays();
...
function _buildPayConfirmLink(s, owe) {
  ...
  const cfg = SD.config || {};
  const bd  = cfg.bankDetails || {};
  const base = 'https://school.edubloom.com.ng/pay-confirm.html';
  const p = new URLSearchParams({
    school:     schoolId || '',
    sid:        s.id || s.studentId || '',
    sn:         s.name || '',
    amt:        String(Math.round(owe)),
    term:       cfg.currentTerm || 'this term',
    bank:       bd.bankName        || '',
    acct:       bd.accountNumber   || '',
    acctname:   bd.accountName     || '',
    schoolname: cfg.schoolName     || 'Your School'
  });
...
function sendReminder(idx) {
  ...
  const payLink = _buildPayConfirmLink(s, owe);
...
Reference:      *${s.name}* _(use your child's name)_`
    : `Please contact the school to arrange payment.`;

  const msg =
`*${sn}* — Fee Payment Notice 🌸
...
━━━━━━━━━━━━━━━━━━━━
✅ *After payment, tap the link below:*
${payLink}

The link lets you upload your bank debit alert or transfer receipt directly to the school. We will verify and update ${s.name.split(' ')[0]}'s record immediately.
━━━━━━━━━━━━━━━━━━━━
...
school.edubloom.com.ng`;
  ...
  if (phone) {
    ...
    navigator.clipboard?.writeText(msg)
      .then(() => toast('📋 No phone — message copied to clipboard.'))
      .catch(() => alert(msg));
  }
}
...
function printReportCard(idx, term) {
  ...
  <div class="watermark">${esc(cfg.schoolName||'Edu-BLOOM')}</div>
  ...
  <div style="display:flex;justify-content:space-between;margin-top:10px;font-size:10.5px;background:#f9fafb;padding:6px 8px;border-radius:5px;border:1px solid #e5e7eb;">
    ...
    <div>🏫 <b>${esc(cfg.schoolName||'')}</b> — Powered by Edu-BLOOM 🌸</div>
  </div>
...
function editStaff(idx) {
  ...
  const html = `
    <div style="display:flex;flex-direction:column;gap:0.5rem;">
      ...
      <label>Role</label>
      <select id="est-role">${['Class Teacher','Subject Teacher','Bursar','Principal','Proprietor'].map(r=>`<option value="${r}" ${s.role===r?'selected':''}>${r}</option>`).join('')}</select>
...
function renderSupport(){
  ...
  contactEl.innerHTML=`
    <div style="display:flex;align-items:center;gap:0.75rem;padding:0.4rem 0;">
      ...
      <button class="btn-wa btn-sm" onclick="window.open('https://wa.me/${(agent.phone||'2348145073941').replace(/\D/g,'')}?text=${encodeURIComponent('Hello '+agent.name+', I need help with Edu-BLOOM.')}','_blank')">📲 Chat</button>
    </div>`;
}
...
function updateLogoBadges(logoUrl) {
  ...
  if (navName) navName.textContent = SD.config.schoolName || 'Edu-BLOOM';
  if (logoUrl) {
    ...
    if (key && students.length > 0) {
      try {
        ...
        const insight = await BloomAgents._gemini(
          `You are Edu-BLOOM's Finance AI for a Nigerian school.
${richCtx}
    ...
    if (key) {
      try {
        const brief = await BloomAgents._gemini(
          `You are Edu-BLOOM's Principal AI for a Nigerian school called "${config.schoolName||'this school'}".
  ...
  runMorningCheck: function(forced) {
    ...
    toAlert.forEach(function(s, i) {
      ...
      const fullMsg = encodeURIComponent(
        ...
        '*' + school + '* 🌸\n\n' +
        msgBody + '\n\n' +
        '— Edu-BLOOM Morning Alert System'
      );
