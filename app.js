...
function startApp(){
  ...
  // Silent background sync — no waiting, no UI block
...
// ── Boot — Offline-First ───────────────────────────────────────────────────
// App loads from localStorage in milliseconds — no network needed.
// Firestore sync happens silently in background when network is available.
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(saved){
    try{
      ...
      // Load everything from localStorage — instant, zero network
      SD.config=loadLocal('config',{});
      SD.students=loadLocal('students',[]);
      SD.staff=loadLocal('staff',[]);
      ...
      // Silent background sync — pull fresh data from Firestore if online
      // No prompts, no UI block, happens automatically
      setTimeout(()=>SQ.silentPull(),2000);
      // Also flush any queued writes
      setTimeout(()=>SQ.flush(),500);
      return;
    }catch(e){
      console.warn('Boot parse error, clearing bad auth:', e);
      localStorage.removeItem('p_auth');sessionStorage.removeItem('p_auth');
    }
  }
