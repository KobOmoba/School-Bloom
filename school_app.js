...
function go(tab){
  ...
  const fn={revenue:renderRevenue,students:renderStudentList,staff:renderStaff,sports:loadSports,arts:renderArts,music:renderMusic,health:renderHealth,alumni:renderAlumni,expenses:renderExpenses,finance:checkFinance,comms:renderComms,analytics:renderAnalytics,security:()=>{},support:renderSupport,settings:loadSettings,opps:renderOpps,scorecard:renderScorecard,guarantee:renderGuarantee};
  if(fn[tab])fn[tab]();
}
...
// ══════════════════════════════════════════════════════════════════
// 🏆 EXAM PASS GUARANTEE — v2
// ══════════════════════════════════════════════════════════════════

async function renderGuarantee(){
  const statusFilter = ($('g-filter-status')?.value||'');
  const termFilter   = ($('g-filter-term')?.value||'');
  const list = $('guarantee-list');
  if(list) list.innerHTML = '<p style="text-align:center;color:var(--sub);padding:1.5rem;">Loading...</p>';

  // Populate student dropdown
  const sel = $('g-student-select');
  if(sel && SD.students){
    const opts = SD.students
      .filter(s=>s.status!=='withdrawn')
      .sort((a,b)=>a.name.localeCompare(b.name))
      .map(s=>`<option value="${s.id}">${esc(s.name)} — ${esc(s.class||'')}</option>`)
      .join('');
  ...
  try{
    ...
    if(termFilter)   q = q.where('term','==',termFilter);
    const snap = await q.get();
    enrollments = snap.docs.map(d=>({id:d.id,...d.data()}));
  }catch(e){
    if(list) list.innerHTML='<p style="color:#dc2626;text-align:center;padding:1rem;">⚠️ Could not load. Check connection.</p>';
    return;
  }
  ...
  const refundAmt= enrollments.filter(e=>e.refundEligible&&e.status!=='refunded').reduce((s,e)=>s+(e.refundAmount||0),0);

  const summaryEl = $('g-summary');
  if(summaryEl) summaryEl.innerHTML = [
    ['Total Enrolled', total, '#2563eb'],
    ['Active', active, '#0d9488'],
    ['Passed ✅', passed, '#16a34a'],
    ['Refund Due ⚠️', refundDue, '#dc2626'],
    ['Revenue', '₦'+fmt(revenue), '#7c3aed'],
    ['Refund Liability', '₦'+fmt(refundAmt), '#ea580c'],
  ].map(([label,val,color])=>`
    <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:0.7rem;text-align:center;">
      <div style="font-size:1.1rem;font-weight:700;color:${color};">${val}</div>
      <div style="font-size:0.7rem;color:var(--sub);margin-top:2px;">${label}</div>
    </div>`).join('');

  if(!enrollments.length){
    if(list) list.innerHTML='<p style="text-align:center;color:var(--sub);padding:2rem;">No enrollments yet. Add your first above.</p>';
    return;
  }

  if(list) list.innerHTML = enrollments.map(e=>{
    const statusColor = {active:'#2563eb',passed:'#16a34a',failed:'#dc2626',refunded:'#6b7280'}[e.status]||'#6b7280';
    const completionBar = `
      <div style="background:#e2e8f0;border-radius:4px;height:6px;margin:4px 0;">
        <div style="background:${e.completionRate>=80?'#16a34a':'#f59e0b'};width:${Math.min(e.completionRate||0,100)}%;height:6px;border-radius:4px;transition:width 0.3s;"></div>
      </div>
      <div style="font-size:0.68rem;color:var(--sub);">${e.completionRate||0}% complete · ${e.mocksTaken||0}/3 mocks</div>`;

    const refundBadge = e.refundEligible && e.status==='failed'
      ? `<span style="background:#fef2f2;border:1px solid #fecaca;color:#dc2626;border-radius:12px;padding:2px 8px;font-size:0.7rem;font-weight:700;">⚠️ REFUND DUE — ₦${fmt(e.refundAmount||0)}</span>`
      : '';

    const actions = e.status==='failed' && e.refundEligible
      ? `<button onclick="markRefunded('${e.id}')" style="background:#dc2626;color:#fff;border:none;border-radius:6px;padding:4px 12px;font-size:0.74rem;cursor:pointer;margin-top:6px;">✅ Mark Refunded</button>`
      : e.status==='active'
      ? `<button onclick="checkGuaranteeOutcome('${e.id}')" style="background:var(--brand);color:#fff;border:none;border-radius:6px;padding:4px 12px;font-size:0.74rem;cursor:pointer;margin-top:6px;">🔄 Check Outcome</button>
         <button onclick="logMock('${e.id}')" style="background:#f1f5f9;border:1px solid var(--border);border-radius:6px;padding:4px 10px;font-size:0.74rem;cursor:pointer;margin-top:6px;color:var(--text);">📝 Log Mock Exam</button>`
      : '';

    return`<div class="deal" style="border-left:3px solid ${statusColor};margin-bottom:0.7rem;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:4px;">
        <div>
          <div class="dn">${esc(e.studentName)}</div>
          <div class="dm">${esc(e.subject)} · ${esc(e.classArm||'')} · ${esc(e.term||'')}</div>
          <div class="dm">Target: ${e.targetScore||50} · Paid: ₦${fmt(e.amountPaid||0)}</div>
        </div>
        <span style="background:${statusColor}22;color:${statusColor};border-radius:12px;padding:2px 10px;font-size:0.72rem;font-weight:700;white-space:nowrap;">${e.status?.toUpperCase()}</span>
      </div>
      ${completionBar}
      ${refundBadge}
      <div>${actions}</div>
...
function prefillGuaranteeClass(){
  ...
  if(s && $('g-class')) $('g-class').value = s.class||'';
}

async function enrollGuarantee(){
  const sid     = $('g-student-select')?.value;
  ...
  if(!amount)  return alert('Enter amount paid.');
  if(!db||!navigator.onLine) return alert('Must be online to enroll.');

  const student = SD.students?.find(s=>s.id===sid);
  if(!student)  return alert('Student not found.');

  const term = SD.config?.currentTerm||'Term 1';
  const session = SD.config?.currentSession||'2025/2026';

  // Check not already enrolled for same subject + term
  const existing = await db.collection('exam_enrollments')
    ...
    .where('term','==',term)
    .get();
  if(!existing.empty) return alert(`${student.name} is already enrolled for ${subject} this term.`);

  const enrollment = {
    schoolId, studentId: sid,
    studentName: student.name,
    classArm: cls, subject, term, session,
    targetScore: target, amountPaid: amount,
    status: 'active',
    refundEligible: false,
    refundAmount: Math.round(amount * 0.7),
    completionRate: 0, mocksTaken: 0,
    enrolledAt: new Date(), refundIssuedAt: null, passedAt: null
  };

  try{
    await db.collection('exam_enrollments').add(enrollment);
    toast(`✅ ${student.name} enrolled for ${subject} guarantee!`);
    if($('g-student-select')) $('g-student-select').value='';
    if($('g-subject'))  $('g-subject').value='';
    if($('g-amount'))   $('g-amount').value='';
    if($('g-class'))    $('g-class').value='';
    renderGuarantee();
  }catch(e){ alert('Error: '+e.message); }
}

async function logMock(enrollId){
  if(!confirm('Confirm this student has completed a mock exam?')) return;
  try{
...
async function checkGuaranteeOutcome(enrollId){
  try{
    ...
    // Pull the actual score from scores collection
    ...
    if(scoreSnap.empty){
      return alert('No score recorded yet for this student in ' + e.subject + ' — ' + e.term + '. Record the score first.');
    }

    const scoreDoc = scoreSnap.docs[0].data();
    const total = scoreDoc.total || ((scoreDoc.CA||0)+(scoreDoc.exam||0));
    const passed = total >= (e.targetScore||50);

    const completionMet = (e.completionRate||0) >= 80;
    const mocksMet = (e.mocksTaken||0) >= 3;
    const refundEligible = !passed && completionMet && mocksMet;
    ...
    if(passed){
      toast(`🎉 ${e.studentName} PASSED ${e.subject} with ${total}/${100}!`);
    } else if(refundEligible){
      toast(`⚠️ ${e.studentName} missed target. Refund of ₦${fmt(e.refundAmount)} is due.`);
    } else {
      const reason = !completionMet
        ? `Only ${e.completionRate||0}% modules completed (need 80%)`
        : `Only ${e.mocksTaken||0} mock exams taken (need 3)`;
      toast(`❌ ${e.studentName} failed but NOT refund-eligible: ${reason}`);
    }
    renderGuarantee();
...
async function markRefunded(enrollId){
  if(!confirm('Confirm refund has been issued to the parent?')) return;
  try{
    await db.collection('exam_enrollments').doc(enrollId).update({
      status: 'refunded',
      refundIssuedAt: new Date()
    });
    toast('✅ Refund marked as issued.');
    renderGuarantee();
  }catch(e){ alert('Error: '+e.message); }
}
